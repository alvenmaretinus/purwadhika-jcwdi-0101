...
export default function LoginForm() {
  ...
  const onSubmit = async (values: FormState) => {
    try {
      ...
      const whereClause = encodeURIComponent(`email = '${values.email}' AND password = '${values.password}'`);
      const response = await axios.get(`${loginsApiUrl}?where=${whereClause}`)
      setIsApiLoading(false)
      
      // if correct credentials
      if (response.data.length) {
        alert(`welcome "${response.data[0]?.email}"`)
      } else {
      ...
      {(props: FormikProps<FormState>) => {
        ...
        return (
          <Form>
            ...
            <button type="submit" className="border border-solid border-black">{isApiLoading ? 'loading' : 'login'}</button>
          </Form>
        )
      }}
    </Formik>
