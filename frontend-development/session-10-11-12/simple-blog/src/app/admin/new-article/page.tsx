...
const ArticleSchema = Yup.object({
  ...
  slug: Yup.string()
    .matches(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, 'Slug must be URL-friendly (lowercase, numbers, hyphens)')
...
export default function NewArticlePage() {
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      ...
      try {
        ...
        if (response.ok) {
          alert(`Article "${values.title}" saved!`);
          resetForm();
        ...
        alert(`Something went wrong during article creation.`);
      }
  ...
  useEffect(() => {
    ...
    if (loggedInUserName) {
      formik.setFieldValue('author', loggedInUserName);
    }
  }, []);
