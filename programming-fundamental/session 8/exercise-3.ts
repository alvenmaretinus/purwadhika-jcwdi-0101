...
class Player {
  ...
  showStatus() {
    console.log(`${this.name} (Health => ${this.health}, Power => ${this.power}, Speed => ${this.speed})`);
  }

  cloneWithRandomStatsMultiplier(): Player {
    const multiplier = (Math.floor(Math.random() * (150 - 10 + 1)) + 10) / 100;
    return new Player('Cloned player', multiplier * this.health, multiplier * this.power, multiplier * this.speed);
  }
}
